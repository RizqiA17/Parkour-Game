...
public class PlayerMovement : Utilities
{
    [Header("State")]
    [HideInInspector] public bool isRolling;
    private bool isGrounded;
    ...
    private void Move()
    {
        ...
        currentVelocity = new Vector2(velocity.x, velocity.z).normalized.magnitude;
    ...
    {
        charController.height = SmoothTransitionFloat(heightBase, height, .5f);
    ...
    private void Sliding()
    {
        ...
        if (Physics.Raycast(ray, out hit, 1f, slideLayer))
        {
            ...
            foreach (var item in slider)
            {
                if (obj == item)
                {
                    float angel = Mathf.SmoothDampAngle(transform.eulerAngles.y, obj.transform.eulerAngles.y + 90, ref currentVelocity, rotationSmoothTime);
                    transform.rotation = Quaternion.Euler(0, angel, 0);

                    //slideSpeed = obj.transform.rotation.eulerAngles.x / 90 * gravityForce;
                    //velocity.z = SmoothTransitionFloat(velocity.z, velocity.z + slideSpeed);
                    velocity.x = SmoothTransitionFloat(velocity.x, velocity.x - slideSpeed);
                    ...
                    gravity = Mathf.Abs(velocity.x) * 5f;

                    if (Input.GetButtonDown("Jump"))
                    {
                        velocity.y = (Mathf.Abs(velocity.x) - 1) * -2.5f;
                        animator.CrossFade(animJumpRun, 0.1f);
                    }
                    else Gravity();
